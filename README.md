...
| `INTERNAL_REFERER_SUFFIX` | `.your-tailnet.ts.net` | Hostname suffix that marks a `Referer` as internal. Required in both modes, and never empty: an empty suffix would make every `Referer` internal, so the proxy refuses to start. Trimmed and lowercased when read. With a leading dot it matches any host that ends with it; without one it matches that host and its subdomains, not `notyour-tailnet.ts.net` |
