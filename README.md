...
| `INTERNAL_BASE_URL` | `https://misskey.your-tailnet.ts.net` | Redirect target for internal media callers. With `MEDIA_MODE=redirect` it is checked at startup: an `http(s)` origin only, no userinfo, query, fragment or path, and plain ASCII (write an IDN host as punycode) |
| `INTERNAL_REFERER_SUFFIX` | `.your-tailnet.ts.net` | Hostname suffix that marks a `Referer` as internal. Required in both modes, and never empty: an empty suffix would make every `Referer` internal, so the proxy refuses to start |
